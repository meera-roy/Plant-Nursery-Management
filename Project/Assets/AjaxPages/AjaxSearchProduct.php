...
include("../Connection/Connection.php");
    ...
    if (isset($_GET["action"])) {
        ...
        $sqlQry = "SELECT * from tbl_product p inner join tbl_type sc on sc.type_id=p.type_id inner join tbl_shop k on k.shop_id=p.shop_id where true ";
        ...
        if ($_GET["type"]!=null) {
            ...
            $sqlQry = $sqlQry." AND sc.type_id IN(".$type.")";
        }
...
<div class="col-md-3 mb-2">
                            <div class="card-deck">
                                <div class="card border-secondary">
                                    <img src="../Assets/Files/User/<?php echo $rowS["product_image"]; ?>" class="card-img-top" height="250">
                                    <div class="card-img-secondary">
                                        <h6  class="text-light bg-info text-center p-2"><?php echo $rowS["product_name"]; ?></h6>
                                    </div>
                                    <div class="card-body">
                                        <h5 class="card-title text-danger" align="center">
                                            Price : <?php echo $rowS["product_rate"]; ?>/-
                                        </h5>
                                        <!-- <p align="center">
                                            <?php// echo $rowS["type_name"]; ?><br>
                                        </p>  -->
                                        <?php
                                        ...
                                        <?php
											...
											$stocka = "select sum(booking_quantity) as stock from tbl_cart where product_id = '" . $rowS["product_id"] . "'";
											$result2a =  mysql_query($stocka);
                            				$row2a= mysql_fetch_array($result2a);
											
											$stock = $row2["stock"] - $row2a["stock"];
											?>
                                            <p align="center" style="color:#dc3545;font-size:15px">Qty <?php echo $stock; ?>-Left</p>
                                            <?php
											
                                                if ($stock > 0) {
                                        ?>
                                        <a href="javascript:void(0)" onclick="addCart('<?php echo $rowS['product_id']; ?>')" class="btn btn-block">Add to Cart</a>
                                        <?php
                                        } else if ($stock == 0) {
                                        ?>
                                        ...
                                        ?>
                                        <a href="ViewDetails.php?did=<?php echo $rowS["product_id"]; ?>" class="btn btn-warning btn-block">View More</a>
                                    </div>
                                </div>
                            </div>
                        </div>
